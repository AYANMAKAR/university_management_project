...
const StudentExamMarks = ({ situation }) => {
    ...
    useEffect(() => {
        if (userDetails && userDetails.sclass_name && situation === "Student") {
    ...
    const changeHandler = (event) => {
        const selectedSubject = subjectsList.find(
            (subject) => subject.sub_name === event.target.value
        );
                    ...
                    >
                        ...
                        >
                            <Stack spacing={1} sx={{ mb: 3 }}>
                                ...
                                {currentUser.teach_subject_id &&
                                    <Typography variant="h4">
                                        Subject Name: {currentUser.teach_subject_id?.sub_name}
                                    </Typography>
                                }
                            </Stack>
                                            ...
                                            >
                                                {subjectsList ?
                                                    subjectsList.map((subject, index) => (
                                                        <MenuItem key={index} value={subject.sub_name}>
                                                            {subject.sub_name}
                                                        </MenuItem>
                                                    ))
                                                    :
